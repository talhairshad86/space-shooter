...
@onready var shield: MeshInstance3D = $Shield/Sphere
...
func shield_hit(value):
	# flash the shield (set a timer to change the alpha value continously)
...
func _on_shield_area_entered(area: Area3D) -> void:
	var bulllet_impact = area.is_in_group("ufo_bullet")
	var enemy_impact = area.is_in_group("enemy")
	if bulllet_impact or enemy_impact:
